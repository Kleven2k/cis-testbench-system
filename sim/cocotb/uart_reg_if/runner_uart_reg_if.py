"""
Runner for uart_reg_if cocotb tests using Icarus Verilog.

Usage:
    python runner_uart_reg_if.py               # run all tests
    python runner_uart_reg_if.py test_read_full_response  # run one test
"""
...
SOURCES = [
    ...
    REPO_ROOT / "hdl" / "rtl" / "uart" / "uart_reg_if.sv",
    SIM_DIR   / "uart_reg_if_tb_wrapper.sv",
]
...
def run(testcase=None):
    ...
    # cocotb is installed under system Python 3.11, not the active venv.
    # Use cocotb-config to locate everything from that installation.
    ...
    cocotb_site   = cocotb_libs.parents[1]  # …/Python311/Lib/site-packages

    env = os.environ.copy()
    env["PYGPI_PYTHON_BIN"]    = cocotb_python
    env["PYTHONPATH"]          = str(SIM_DIR) + os.pathsep + str(cocotb_site)
    env["COCOTB_TEST_MODULES"] = "test_uart_reg_if"
