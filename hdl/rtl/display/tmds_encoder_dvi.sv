...
module tmds_encoder_dvi (
    ...
    always_comb begin
        ...
        for (i=0; i<7; i=i+1) begin
            /* verilator lint_of ALWCOMBORDER */
    ...
    always_ff @(posedge clk_pix) begin
        if (de == 0) begin  // send control data in blanking interval
            case (ctrl_in)  // ctrl sequences (always have 7 transitions)
                ...
                2'b10:   tmds <= 10'b1010101011;
                default: tmds <= 10'b1010101011;
            endcase
            bias <= 5'sb00000;
        end else begin  // send pixel colour data (at most 5 transistions)
            if (bias == 0 || balance == 0) begin    // no prior bias or dispersity
                if (enc_qm[8] == 0) begin
