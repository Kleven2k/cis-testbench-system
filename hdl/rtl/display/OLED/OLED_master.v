...
module OLED_master (
    ...
    // SPLASH screen text
    localparam  splash_str1="Exposure:       ", splash_str1_len=16;
    reg [8*16-1:0] splash_str2 = "0000 us         ";
    //localparam  splash_str2="8 us            ", splash_str2_len=16;
    localparam  splash_str3="                ", splash_str3_len=16;
    localparam  splash_str4="                ", splash_str4_len=16;
    // ALPHA screen text
    localparam  alpha_str1="       UIO      ", alpha_str1_len=16;
    localparam  alpha_str2="  CIS Testbench ", alpha_str3_len=16;
    localparam  alpha_str3="                ", alpha_str2_len=16;
    localparam  alpha_str4=" Initializing...", alpha_str4_len=16;
    ...
    function automatic [7:0] to_ascii(input [3:0] d);
        to_ascii = 8'd48 + d;
    endfunction

    // Update splash_str2 when exposure changes
    reg [15:0] exposure_prev;
    reg exposure_changed_d;

    always @(posedge clk) begin 
        exposure_prev <= exposure_us;
        exposure_changed_d <= (exposure_prev != exposure_us);
    end 

    wire exposure_pulse = (exposure_prev != exposure_us) && !exposure_changed_d;
    reg [15:0] exposure_latched;

    always @(posedge clk) begin 
        if (state == ActiveWait && exposure_pulse) begin 
            exposure_latched <= exposure_us;
        end 
    end 
    always @(posedge clk) begin
        if (state == ActiveWait && exposure_pulse) begin
            splash_str2[15*8 +: 8] <= to_ascii((exposure_latched / 1000) % 10);
            splash_str2[14*8 +: 8] <= to_ascii((exposure_latched / 100)  % 10);
            splash_str2[13*8 +: 8] <= to_ascii((exposure_latched / 10)   % 10);
            splash_str2[12*8 +: 8] <= to_ascii(exposure_latched % 10);
        end
    end 

    wire exposure_changed = (exposure_prev != exposure_us);
    ...
    always@(posedge clk)
        case (state)
            ...
            ActiveWait: begin // hold until ready, then accept input
                if (exposure_pulse) begin 
                    // rewrite slash screen with new value
                    write_base_addr <= 'b0;
                    screen_select <= SPLASH;
                    after_state <= ActiveUpdateSplash;
                    state <= WriteWait;
                end 
                else if (rst && disp_off_ready) begin
                    disp_off_start <= 1'b1;
                    state <= Done;
                end
            end
